...
def make_bench_periodic_box_plot(fileprefix,report) -> str:

    tests = []
    
    for r in report["results"]:
        if r["type"] == "Benchmark" and r["name"] == "benchmark periodic box sph":
            tests.append(r)
    ...
    plt.savefig("figures/"+fileprefix+"bench_periodic_box.pdf")


    return r"""

    \subsection{Periodic box SPH perf}

    \begin{figure}[ht!]
    \center
    \includegraphics[width=1\textwidth]{"""+"figures/"+fileprefix+"bench_periodic_box.pdf"+r"""}
    \caption{TODO}
    \end{figure}
...
def make_bench_periodic_box_report(fileprefix : str, report) -> str:
    buf = ""

    buf += make_bench_periodic_box_plot(fileprefix,report)
