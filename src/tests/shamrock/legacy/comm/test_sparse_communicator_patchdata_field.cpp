...
SchedulerPatchList make_plist(std::mt19937 & eng, u32 max_patch_per_node, u32 nb_part){
    ...
    for (u32 i = 0; i < nb_patch_test; i++) {
        ...
        Patch p;
        p.id_patch = i + u64(shammpi::world_rank())*max_patch_per_node;
...
BenchResult benchmark_comm(std::mt19937 & eng, u32 max_patch_per_node, u32 nb_part){
    ...
    for(const Patch & p : plist.local){
        send_vec.push_back({
            plist.id_patch_to_global_idx[p.id_patch],
            dist_pb(eng)
            });
    ...
    mpi::reduce(&xchg_fetch_data,&sum_byte_fetch, 1, get_mpi_type<u64>(), MPI_SUM, 0, MPI_COMM_WORLD);
    mpi::reduce(&xchg_comm_data,&sum_byte_comm, 1, get_mpi_type<u64>(), MPI_SUM, 0, MPI_COMM_WORLD);
...
TestStart(Benchmark, "core/comm/sparse_communicator_patchdata_field:", func_name, -1){
    ...
    std::mt19937 eng (0x2525 + shammpi::world_rank());
