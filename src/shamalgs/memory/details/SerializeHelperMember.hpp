// -------------------------------------------------------//
//
// SHAMROCK code for hydrodynamics
// Copyright(C) 2021-2023 Timothée David--Cléris <timothee.david--cleris@ens-lyon.fr>
// Licensed under CeCILL 2.1 License, see LICENSE for more information
//
// -------------------------------------------------------//

#pragma once

#include "shambase/memory.hpp"
#include "shambase/sycl.hpp"
#include "shambase/type_aliases.hpp"
#include "shamsys/NodeInstance.hpp"
#include <hipSYCL/sycl/libkernel/vec.hpp>
#include <memory>
#include <type_traits>

namespace shamalgs::details {

    template<class T>
    class SerializeHelperMember {
        static constexpr u64 szrepr = sizeof(T);
        inline void store_val(u8* ptr_store, T val){
            T* ptr = (T*) ptr_store;
            *ptr = val;
        }
        inline T load(u8* ptr_load){
            T* ptr = (T*) ptr_load;
            return *ptr;
        }
    };

    template<class T>
    class SerializeHelperMember<sycl::vec<T, 2>> {
        
        static constexpr i32 n = 2;
        static constexpr u64 szrepr = sizeof(T)*n;

        inline void store_val(u8* ptr_store, sycl::vec<T, n> val){
            T* ptr = (T*) ptr_store;
            ptr[0] = val.x();
            ptr[1] = val.y();
        }
        inline sycl::vec<T, n> load(u8* ptr_load){
            T* ptr = (T*) ptr_load;
            return sycl::vec<T, n>{ptr[0],ptr[1]};
        }
    };

    template<class T>
    class SerializeHelperMember<sycl::vec<T, 3>> {
        
        static constexpr i32 n = 3;
        static constexpr u64 szrepr = sizeof(T)*n;

        inline void store_val(u8* ptr_store, sycl::vec<T, n> val){
            T* ptr = (T*) ptr_store;
            ptr[0] = val.x();
            ptr[1] = val.y();
            ptr[2] = val.z();
        }
        inline sycl::vec<T, n> load(u8* ptr_load){
            T* ptr = (T*) ptr_load;
            return sycl::vec<T, n>{ptr[0],ptr[1],ptr[2]};
        }
    };
    
    template<class T>
    class SerializeHelperMember<sycl::vec<T, 4>> {
        
        static constexpr i32 n = 4;
        static constexpr u64 szrepr = sizeof(T)*n;

        inline void store_val(u8* ptr_store, sycl::vec<T, n> val){
            T* ptr = (T*) ptr_store;
            ptr[0] = val.x();
            ptr[1] = val.y();
            ptr[2] = val.z();
            ptr[3] = val.w();
        }
        inline sycl::vec<T, n> load(u8* ptr_load){
            T* ptr = (T*) ptr_load;
            return sycl::vec<T, n>{ptr[0],ptr[1],ptr[2],ptr[3]};
        }
    };

    template<class T>
    class SerializeHelperMember<sycl::vec<T, 8>> {
        
        static constexpr i32 n = 8;
        static constexpr u64 szrepr = sizeof(T)*n;

        inline void store_val(u8* ptr_store, sycl::vec<T, n> val){
            T* ptr = (T*) ptr_store;
            ptr[0] = val.s0();
            ptr[1] = val.s1();
            ptr[2] = val.s2();
            ptr[3] = val.s3();
            ptr[4] = val.s4();
            ptr[5] = val.s5();
            ptr[6] = val.s6();
            ptr[7] = val.s7();
        }
        inline sycl::vec<T, n> load(u8* ptr_load){
            T* ptr = (T*) ptr_load;
            return sycl::vec<T, n>{ptr[0],ptr[1],ptr[2],ptr[3],ptr[4],ptr[5],ptr[6],ptr[7]};
        }
    };

    template<class T>
    class SerializeHelperMember<sycl::vec<T, 16>> {
        
        static constexpr i32 n = 16;
        static constexpr u64 szrepr = sizeof(T)*n;

        inline void store_val(u8* ptr_store, sycl::vec<T, n> val){
            T* ptr = (T*) ptr_store;
            ptr[0] = val.s0();
            ptr[1] = val.s1();
            ptr[2] = val.s2();
            ptr[3] = val.s3();
            ptr[4] = val.s4();
            ptr[5] = val.s5();
            ptr[6] = val.s6();
            ptr[7] = val.s7();
            ptr[8] = val.s8();
            ptr[9] = val.s9();
            ptr[10] = val.sA();
            ptr[11] = val.sB();
            ptr[12] = val.sC();
            ptr[13] = val.sD();
            ptr[14] = val.sE();
            ptr[15] = val.sF();
        }
        inline sycl::vec<T, n> load(u8* ptr_load){
            T* ptr = (T*) ptr_load;
            return sycl::vec<T, n>{ptr[0],ptr[1],ptr[2],ptr[3],ptr[4],ptr[5],ptr[6],ptr[7],
            ptr[8],ptr[9],ptr[10],ptr[11],ptr[12],ptr[13],ptr[14],ptr[15]};
        }
    };

} // namespace shamalgs::details