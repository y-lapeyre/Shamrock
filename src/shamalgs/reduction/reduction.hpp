...
namespace shamalgs::reduction {
    ...
    template<class T> bool equals(sycl::buffer<T> &buf1, sycl::buffer<T> &buf2){
    ...
    template<class T> bool equals_ptr(const std::unique_ptr<sycl::buffer<T>> &buf1, const std::unique_ptr<sycl::buffer<T>> &buf2){
        bool same_alloc = bool(buf1) == bool(buf2);
