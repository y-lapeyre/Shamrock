...
template<>
std::vector<std::unique_ptr<shamrock::patch::PatchDataLayer>> InterfaceVolumeGenerator::
    append_interface<f32_3>(
        sycl::queue &queue,
    ...
    for (auto &p : pdat_vec) {
        p = std::make_unique<PatchDataLayer>(pdat.get_layout_ptr());
    }
