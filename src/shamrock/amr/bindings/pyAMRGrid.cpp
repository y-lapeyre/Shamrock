...
Register_pymod(pyamrgridinit){
    ...
    py::class_<Grid>(m, "AMRGrid")
        ...
        .def("make_base_grid",
            [](Grid & grid, std::array<u64,3> min, std::array<u64,3> cell_size, std::array<u32, 3> cell_count){
            
            grid.make_base_grid(u64_3{min[0],min[1],min[2]}, u64_3{cell_size[0],cell_size[1],cell_size[2]}, cell_count);
        });
