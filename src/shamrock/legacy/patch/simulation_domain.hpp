...
class SimulationDomain {public:
    ...
    inline void check_boundary(){
        if (boundary_type == PeriodicShearing) {
            throw shamrock_exc("[SimulationDomain] Boundary CD : Shearing periodic mode not implemented");
        }

        if (boundary_type == Fixed) {
            throw shamrock_exc("[SimulationDomain] Boundary CD : Dirichelt mode not implemented");
        }

        if (boundary_type == Ghost) {
            throw shamrock_exc("[SimulationDomain] Boundary CD : Ghost mode not implemented");
        }

        if (boundary_type == FixedGradient) {
            throw shamrock_exc("[SimulationDomain] Boundary CD : FixedGradient mode not implemented");
        }

        if (boundary_type == AntiPeriodic) {
            throw shamrock_exc("[SimulationDomain] Boundary CD : AntiPeriodic mode not implemented");
        }
    }
    ...
    inline vec get_periodicity_vector() const {
        ...
        if (boundary_type == Periodic || boundary_type == PeriodicShearing){
            ...
            throw shamrock_exc("[SimulationDomain] Can not set box size with free boundary conditions");
        }
