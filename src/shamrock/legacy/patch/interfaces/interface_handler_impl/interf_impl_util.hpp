    ...
    namespace generator {
        ...
        struct GeneratorBuffer{
            ...
            explicit GeneratorBuffer(PatchScheduler & sched) : 
                ...
                if (local_pcount == 0){
                    throw shambase::make_except_with_loc<std::runtime_error>("local patch count is zero this function can not run");
                }
        ...
        inline sycl::buffer<impl::CommInd<flt>, 2> compute_buf_interact(
            ...
            sycl::vec<flt, 3> test_patch_offset, bool has_off,
            ...
            logger::debug_sycl_ln("InterfaceFinder", "searching interfaces offset=",test_patch_offset,"loc count =",local_pcount,"g count=",global_pcount);
                    ...
                    cgh.parallel_for(sycl::range<1>(local_pcount), [=](sycl::item<1> item) {
                        ...
                        for (u64 test_patch_idx = 0; test_patch_idx < cnt_patch; test_patch_idx++) {
...

                            {
                                ...
                                bool is_not_itself = (is_off_not_bull) || (test_patch_id != cur_patch_id);
