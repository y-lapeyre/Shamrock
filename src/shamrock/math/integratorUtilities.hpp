...
namespace shamrock {
    ...
    inline void
    fields_forward_euler(PatchScheduler &sched, u32 field_idx, u32 derfield_idx, flt dt) {
    ...
    inline void fields_leapfrog_corrector(
        PatchScheduler &sched, u32 field_idx, u32 derfield_idx, u32 derfield_old_idx, flt hdt) {
        using namespace shamrock::patch;
        sched.for_each_patch_data([&](u64 id_patch, Patch cur_p, PatchData &pdat) {
    ...
    template<class T>
    inline void
    fields_apply_periodicity(PatchScheduler &sched, u32 field_idx, std::pair<T, T> box) {
        using namespace shamrock::patch;
        sched.for_each_patch_data([&](u64 id_patch, Patch cur_p, PatchData &pdat) {
    ...
    template<class T>
    inline void fields_swap(PatchScheduler &sched, u32 field_idx1, u32 field_idx2) {
        using namespace shamrock::patch;
        sched.for_each_patch_data([&](u64 id_patch, Patch cur_p, PatchData &pdat) {
            utilities::swap_fields(shamsys::instance::get_compute_queue(),
...
} // namespace shamrock::integrators