...
enum PositionprecMode { xyz32, xyz64 };

namespace shamrock::patch {
    class PatchDataLayout {

        template <class T> class FieldDescriptor {
            public:
            std::string name;
            u32 nvar;
            using field_T = T;
        ...
        public:
        template <class T> void add_field(std::string field_name, u32 nvar) {
            bool found = false;
        ...
        template <class T> inline FieldDescriptor<T> get_field(std::string field_name) {
        ...
        template <class T> inline u32 get_field_idx(std::string field_name) {
        ...
        template <class Functor> inline void for_each_field_any(Functor &&func) {
            for (auto &f : fields) {
