...
namespace shamsys::comm::details {
    ...
    template<class T> 
    class CommDetails<PatchDataField<T>>{public:
        u32 obj_cnt;
        std::string name;
        u32 nvar;
    ...
    class CommBuffer<PatchDataField<T>,comm_mode>{
        ...
        CommBuffer(CommBuffer<sycl::buffer<T>,comm_mode> && buf_comm,
        CommDetails<PatchDataField<T>> && details) : buf_comm(std::move(buf_comm)), details(std::move(details)){}
        
        public:

        inline CommBuffer(CommDetails<PatchDataField<T>> det): 
            details(det),
            buf_comm {det._get_buf_details()} {}

        inline CommBuffer( PatchDataField<T> & obj_ref) : 
            details(obj_ref), 
            buf_comm(
                *obj_ref.get_buf(), 
                CommDetails<PatchDataField<T>>{obj_ref}._get_buf_details()
                )
            {}
        ...
        inline PatchDataField<T> copy_back(){
            ...
            return PatchDataField<T>{
                std::move(buf), 
                details.obj_cnt,details.name,details.nvar};
        }
        ...
        inline static PatchDataField<T> convert(CommBuffer && buf){
            ...
            return PatchDataField<T>{
                std::move(buf_recov), 
                buf.details.obj_cnt,buf.details.name,buf.details.nvar};
        }
        ...
        inline static CommBuffer irecv_probe(CommRequests & rqs, u32 rank_src, u32 comm_flag, MPI_Comm comm, CommDetails<PatchDataField<T>> details){

            auto recv = CommBuffer<sycl::buffer<T>, comm_mode>::irecv_probe(rqs, rank_src, comm_flag, comm, {});

            u64 cnt_recv = recv.get_details().comm_len;

            if(cnt_recv % details.nvar != 0){
                throw shamutils::throw_with_loc<std::runtime_error>("the received message must be disible by nvar to be received as PatchDataField");
            }
