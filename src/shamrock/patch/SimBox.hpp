...
/**
 * @brief Store the information related to the size of the simulation box to convert patch integer
 * coordinates to floating point ones.
...
class SimulationBoxInfo {
    ...
    public:
    inline SimulationBoxInfo(PatchDataLayout &pdl
                             ,CoordRange<u64_3> patch_coord_bounding_box
    )
        : pdl(pdl)
    ...
    template <class T>
    std::tuple<T, T> partch_coord_to_domain(const Patch &p) const;
    ...
    inline std::tuple<sycl::vec<primtype, 3>, sycl::vec<primtype, 3>>
    get_box(Patch &p) {
...
template <class T>
inline std::tuple<T, T> SimulationBoxInfo::partch_coord_to_domain(const Patch &p
) const {
