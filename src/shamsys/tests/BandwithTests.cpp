...
template<class T> void bench_memcpy_sycl(std::string dset_name, sycl::queue & q1, sycl::queue & q2, u64 max_byte_sz_cnt){
    ...
    for(f64 cnt_ = 1e2; cnt_ < max_obj_cnt; cnt_ *= 1.1){
        ...
        bandwith_GBsm1.push_back((f64(cnt * sizeof(T))/(t.nanosec/1e9)) / f64(1024*1024*1024));
    }
...
template<class T> void bench_memcpy_sycl_host_dev(std::string dset_name, sycl::queue & q1, u64 max_byte_sz_cnt){
...
TestStart(Benchmark, "bandwith-tests/memcpy-sycl", compcompbandwithtest, 1){
    ...
    bench_memcpy_sycl<f32_2>("comp -> comp (f32_2)", get_compute_queue(), get_compute_queue(), max_sz);
    bench_memcpy_sycl<f32_3>("comp -> comp (f32_3)", get_compute_queue(), get_compute_queue(), max_sz);
    bench_memcpy_sycl<f32_4>("comp -> comp (f32_4)", get_compute_queue(), get_compute_queue(), max_sz);
    bench_memcpy_sycl<f32_8>("comp -> comp (f32_8)", get_compute_queue(), get_compute_queue(), max_sz);
    bench_memcpy_sycl<f32_16>("comp -> comp (f32_16)", get_compute_queue(), get_compute_queue(), max_sz);

    bench_memcpy_sycl<f64_2>("comp -> comp (f64_2)", get_compute_queue(), get_compute_queue(), max_sz);
    bench_memcpy_sycl<f64_3>("comp -> comp (f64_3)", get_compute_queue(), get_compute_queue(), max_sz);
    bench_memcpy_sycl<f64_4>("comp -> comp (f64_4)", get_compute_queue(), get_compute_queue(), max_sz);
    bench_memcpy_sycl<f64_8>("comp -> comp (f64_8)", get_compute_queue(), get_compute_queue(), max_sz);
    bench_memcpy_sycl<f64_16>("comp -> comp (f64_16)", get_compute_queue(), get_compute_queue(), max_sz);
...
template<class T> void make_bandwith_matrix(std::string dset_name,sycl::queue & q1, u32 comm_size){
...
TestStart(Benchmark, "bandwith-tests/mpi-pair-comm/bw-matrix", mpi_pair_comm_bw_matrix, -1){
    ...
    make_bandwith_matrix<f32_3>("(f32_3)",shamsys::instance::get_compute_queue(),1024*1024*16/256);
}
