...
class SerialPatchTree{public:
    ...
    inline void detach_buf(){
        if(serial_tree_buf == nullptr) throw std::runtime_error("serial_tree_buf wasn't allocated");
        if(linked_patch_ids_buf == nullptr) throw std::runtime_error("linked_patch_ids_buf wasn't allocated");
