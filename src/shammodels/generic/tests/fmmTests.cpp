...
class FMM_prec_eval{public:
    static T eval_prec_fmm_pot(sycl::vec<T, 3> xi,sycl::vec<T, 3> xj,sycl::vec<T, 3> sa,sycl::vec<T, 3> sb){
        ...
        //compute fmm
        {
            ...
            if constexpr (order >= 1) {phi_val += M_k.t1*a_k.t1;}
            if constexpr (order >= 2) {phi_val += M_k.t2*a_k.t2;}
            if constexpr (order >= 3) {phi_val += M_k.t3*a_k.t3;}
            if constexpr (order >= 4) {phi_val += M_k.t4*a_k.t4;}
            if constexpr (order >= 5) {phi_val += M_k.t5*a_k.t5;}
    ...
    static T eval_prec_fmm_force(sycl::vec<T, 3> xi,sycl::vec<T, 3> xj,sycl::vec<T, 3> sa,sycl::vec<T, 3> sb){
        ...
        //compute fmm
        {
            ...
            if constexpr (order >= 2) {force_val += tensor_to_sycl(dM_k.t2*a_k.t1);}
            if constexpr (order >= 3) {force_val += tensor_to_sycl(dM_k.t3*a_k.t2);}
            if constexpr (order >= 4) {force_val += tensor_to_sycl(dM_k.t4*a_k.t3);}
            if constexpr (order >= 5) {force_val += tensor_to_sycl(dM_k.t5*a_k.t4);}
...
Result_nompi_fmm_testing<flt,morton_mode,fmm_order> nompi_fmm_testing(std::unique_ptr<sycl::buffer<sycl::vec<flt,3>>> & pos_part, u32 reduc_level,flt open_crit){
    ...
    std::unique_ptr<sycl::buffer<vec>> cell_centers = std::make_unique<sycl::buffer<vec>>(rtree.tree_struct.internal_cell_count + rtree.tree_leaf_count);
    std::unique_ptr<sycl::buffer<flt>> cell_lenght = std::make_unique<sycl::buffer<flt>>(rtree.tree_struct.internal_cell_count + rtree.tree_leaf_count);
...
TestStart(Analysis,"models/generic/fmm/fmm_1_gpu_prec", fmm_1_gpu_prec , 1){
    ...
    if(false){
        ...
        std::vector<vec> pos_part_tmp{
            min,min + diff*vec{1./64.,0,0},
            max,max - diff*vec{1./64.,0,0}
        };
...

        {
            ...
            logger::raw_ln("multipoles -> u32 len :",res_u32.grav_multipoles->size(),"u64 len :",res_u64.grav_multipoles->size());
        ...
        if(false){
            ...
            for(u32 i = 0 ; i < res_u64.rtree.tree_struct.internal_cell_count ; i++){
                ...
                if(!same){
                    ...
                    logger::raw_ln("i=",i,"-> diff "
                    
                    , " ", acc_u32_lid[i] ,"|", acc_u64_lid[i], " "
                    , " ", acc_u32_rid[i] ,"|", acc_u64_rid[i], " "
                    , " ", u32{acc_u32_lflag[i]} ,"|", u32{acc_u64_lflag[i]}, " "
                    , " ", u32{acc_u32_rflag[i]} ,"|", u32{acc_u64_rflag[i]}, " ");
            ...
            for(u32 i = 0 ; i < res_u64.rtree.tree_struct.internal_cell_count + res_u64.rtree.tree_leaf_count ; i++){
                bool same = true;
                ...
                if(!same){
                    ...
                    logger::raw_ln("i=",i,"-> diff "
                        , " ", acc_min_u32[i] , acc_min_u64[i] , "  ",acc_max_u32[i] , acc_max_u64[i]);
...
constexpr auto list_npart_test = {
       ...
       2.47707636e+04, 2.91505306e+04, 3.43046929e+04, 
       4.03701726e+04 ,
