...
shambase::DistributedData<std::unique_ptr<shammodels::basegodunov::modules::AMRBlockGraph>>
shammodels::basegodunov::modules::AMRGraphGen<Tvec, TgridVec>::find_AMR_block_graph_links() {
    ...
    shambase::DistributedData<std::unique_ptr<AMRBlockGraph>> block_graph_links;
    ...
    storage.trees.get().for_each([&](u64 id, RTree &tree) {
        ...
        u32 link_cnt = shamalgs::memory::extract_element(q, link_cnt_offsets, mpdat.total_elements);
        ...
        block_graph_links.add_obj(
            id,
            std::make_unique<AMRBlockGraph>(
