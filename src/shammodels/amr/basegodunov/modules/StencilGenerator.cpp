...
void _kernel(
    ...
    case 2: ret = block::StencilElement(block::Levelm1{found_cells[0]}); break;
...
auto shammodels::basegodunov::modules::StencilGenerator<Tvec, TgridVec>::lower_block_slot_to_cell(
    ...
    return block_stencil_el.map<cell_stencil_el_buf>(
        [&](u64 id, block_stencil_el_buf &block_stencil_el_b) {
            return lower_block_slot_to_cell(relative_pos, result_offset, block_stencil_el_b);
        });
}
...
template<class Tvec, class TgridVec>
auto shammodels::basegodunov::modules::StencilGenerator<Tvec, TgridVec>::lower_block_slot_to_cell(
    i64_3 relative_pos, StencilOffsets result_offset, block_stencil_el_buf &block_stencil_el)
    -> cell_stencil_el_buf {
