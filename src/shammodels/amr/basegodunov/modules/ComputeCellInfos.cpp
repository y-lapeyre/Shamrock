...
void shammodels::basegodunov::modules::ComputeCellInfos<Tvec, TgridVec>::compute_aabb() {
    ...
    shamrock::ComputeField<Tscal> block_cell_sizes = utility.make_compute_field<Tscal>("aabb cell size", 1, [&](u64 id) {
        return storage.merged_patchdata_ghost.get().get(id).total_elements;
    });
    shamrock::ComputeField<Tvec> cell0block_aabb_lower = utility.make_compute_field<Tvec>("aabb cell lower", 1, [&](u64 id) {
        return storage.merged_patchdata_ghost.get().get(id).total_elements;
    });
    ...
    storage.merged_patchdata_ghost.get().for_each([&](u64 id, MergedPDat &mpdat){
        ...
        q.submit([&](sycl::handler &cgh) {
            ...
            sycl::accessor bsize{
                shambase::get_check_ref(block_cell_sizes.get_buf(id)), cgh, sycl::write_only, sycl::no_init};
            sycl::accessor aabb_lower{
                shambase::get_check_ref(cell0block_aabb_lower.get_buf(id)), cgh, sycl::write_only, sycl::no_init};
    ...
    storage.cell_infos.set(CellInfos<Tvec, TgridVec>{
        std::move(block_cell_sizes),
        std::move(cell0block_aabb_lower)
    });
