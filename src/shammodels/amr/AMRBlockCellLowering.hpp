...
namespace shammodels::amr {
    ...
    struct BlockCellLowering{
        ...
        inline static constexpr std::array<i32,3> get_relative_offset_vec(){
            ...
            if constexpr(stencil_pos == xm1){
                return std::array<i32,3>{-1,0,0};
            }else 
            if constexpr(stencil_pos == yp1){
                return std::array<i32,3>{0,1,0};
            }else 
            if constexpr(stencil_pos == ym1){
                return std::array<i32,3>{0,-1,0};
            }else 
            if constexpr(stencil_pos == zp1){
                return std::array<i32,3>{0,0,1};
            }else 
            if constexpr(stencil_pos == zm1){
                return std::array<i32,3>{0,0,-1};
            }else { 
                static_assert(shambase::always_false_v<decltype(stencil_pos)>, "non-exhaustive visitor!");
            }
        }
            ...
            )
        {
            ...
            if(offset_lcoord > 0 && offset_lcoord < AMRBlock::block_size){
                return cell::StencilElement{
                    cell::SameLevel{
                        block_id*AMRBlock::block_size + offset_lcoord
                    }
                };
            }
            ...
            return block_stencil_el.visitor_ret<cell::StencilElement>(
                [&](block::SameLevel st){
                    return cell::SameLevel{
                            st.block_idx*AMRBlock::block_size + AMRBlock::get_index({
                                u32(coord_off[0]),
                                u32(coord_off[1]),
                                u32(coord_off[2])
                            })
                ...
                [&](block::Levelm1 st){
                    ...
                    auto neigh_state = cell::Levelm1::STATE(mod_coord[0]*4 + mod_coord[1]*2 + mod_coord[2]);
                ...
                [&](block::Levelp1 st){
                    ...
                    u32 idx_tmp = st.block_child_idxs[child_select]*AMRBlock::block_size +AMRBlock::get_index({
                                u32(coord_off[0]),
                                u32(coord_off[1]),
                                u32(coord_off[2])
                            }) ;
