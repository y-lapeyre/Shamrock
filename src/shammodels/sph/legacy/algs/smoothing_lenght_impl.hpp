...
namespace impl {
    ...
    inline void sycl_init_h_iter_bufs<f64>(
        ...
        queue.submit([&](sycl::handler &cgh) {
            ...
            auto acc_hpart = pdat_merge.get_field<f64>(ihpart).get_buf()->get_access<sycl::access::mode::read>(cgh);
            auto eps = eps_h.get_access<sycl::access::mode::discard_write>(cgh);
            auto h    = hnew.get_access<sycl::access::mode::discard_write>(cgh);
    ...
    class IntSmoothinglengthCompute{public:
        ...
        inline static void sycl_h_iter_step<f64>(
            ...
            queue.submit([&](sycl::handler &cgh) {
                ...
                auto acc_hpart = pdat_merge.get_field<f64>(ihpart).get_buf()->get_access<sycl::access::mode::read>(cgh);
                auto r = pdat_merge.get_field<f64_3>(ixyz).get_buf()->get_access<sycl::access::mode::read>(cgh);
                ...
                cgh.parallel_for<Kernel_Iterh<f64, morton_prec, Kernel>>(range_npart, [=](sycl::item<1> item) {
                    u32 id_a = (u32)item.get_id(0);
        ...
        inline static void sycl_h_iter_omega<f64>(
            ...
            queue.submit([&](sycl::handler &cgh) {
                ...
                cgh.parallel_for<Kernel_Finalize_omega<f64, morton_prec, Kernel>>(range_npart, [=](sycl::item<1> item) {
                    u32 id_a = (u32)item.get_id(0);
