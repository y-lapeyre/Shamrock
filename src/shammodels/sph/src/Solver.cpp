...
shammodels::sph::TimestepLog shammodels::sph::Solver<Tvec, Kern>::evolve_once_substep() {
    ...
    do_predictor_substep(dt_sph); // predict velocity using long range acceleration
    part_killing_step();

    do_substeps(dt_sph, dt_ext); // loop to update velocity using short range acceleration
    ...
    sph_prestep(t_current, dt_sph); // iterate smoothing length
    ...
    if (solver_config.has_field_dtdivv()) {
        ...
        if (solver_config.has_field_divv()) {
            sph::modules::DiffOperators<Tvec, Kern>(context, solver_config, storage).update_divv();
        }

        if (solver_config.has_field_curlv()) {
            sph::modules::DiffOperators<Tvec, Kern>(context, solver_config, storage).update_curlv();
        }
    }

    update_artificial_viscosity(dt_sph);
    ...
    shamlog_debug_ln("sph::BasicGas", "leapfrog corrector");
    utility.fields_leapfrog_corrector<Tvec>(
        ivxyz, iaxyz, storage.old_axyz.get(), vepsilon_v_sq, dt_sph / 2);
    utility.fields_leapfrog_corrector<Tscal>(
        iuint, iduint, storage.old_duint.get(), uepsilon_u_sq, dt_sph / 2);
    ...
    if (solver_config.has_field_alphaAV()) {
        ...
        scheduler().for_each_patchdata_nonempty([&](Patch cur_p, PatchDataLayer &pdat) {
            sham::DeviceBuffer<Tscal> &buf_alpha_av = pdat.get_field<Tscal>(ialpha_AV).get_buf();
            sham::DeviceBuffer<Tscal> &buf_alpha_av_updated
                = alpha_av_updated.get_buf_check(cur_p.id_patch);
    ...
    scheduler().for_each_patchdata_nonempty([&](Patch cur_p, PatchDataLayer &pdat) {
...

        {
            ...
            auto e = q.submit(depends_list, [&](sycl::handler &cgh) {
                ...
                shambase::parallel_for(cgh, pdat.get_obj_cnt(), "compute vsig", [=](i32 id_a) {
                    using namespace shamrock::sph;
    ...
    scheduler().for_each_patchdata_nonempty([&](Patch cur_p, PatchDataLayer &pdat) {
        ...
        buf_axyz.complete_event_state(e);
        vsig_buf.complete_event_state(e);
        sph_dt_buf.complete_event_state(e);
        force_dt_buf.complete_event_state(e);
    });
    ...
    if (shamcomm::world_rank() == 0) {
        logger::info_ln("sph::Model", log_step);
        logger::info_ln(
            "sph::Model", "estimated rate :", dt_sph * (3600 / tstep.elasped_sec()), "(tsim/hr)");
    }
    ...
    solver_config.set_next_dt(next_cfl);
    solver_config.set_time(t_current + dt_sph);
