...
void shammodels::sph::Model<Tvec, SPHKernel>::push_particle_mhd(
    ...
    std::vector<Tvec> &B_on_rho_insert,
    std::vector<Tscal> &psi_on_cs_insert) {
    StackEntry stack_loc{};
    ...
    sched.for_each_local_patchdata([&](const Patch p, PatchData &pdat) {
        ...
        std::vector<Tvec> BOR_acc;
        std::vector<Tscal> POC_acc;
        for (u32 i = 0; i < part_pos_insert.size(); i++) {
            Tvec r    = part_pos_insert[i];
            Tscal u   = part_u_insert[i];
            Tvec v    = part_vel_insert[i];
            Tvec BOR  = B_on_rho_insert[i];
            Tscal POC = psi_on_cs_insert[i];
            if (patch_coord.contain_pos(r)) {
                vec_acc.push_back(r);
                hpart_acc.push_back(part_hpart_insert[i]);
                u_acc.push_back(u);
                vel_acc.push_back(v);
                BOR_acc.push_back(BOR);
                POC_acc.push_back(POC);
            }
        }
