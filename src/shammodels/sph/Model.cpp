    ...
    sched.for_each_local_patchdata([&](const Patch p, PatchData &pdat) {
...

        {
...

         {
            u32 len = B_acc.size();
            PatchDataField<Tvec> &f
                = tmp.get_field<Tvec>(sched.pdl.get_field_idx<Tvec>("B/rho"));
            sycl::buffer<Tvec> buf(B_acc.data(), len);
            f.override(buf, len);
        }

        {
            u32 len                  = psi_acc.size();
            PatchDataField<Tscal> &f = tmp.get_field<Tscal>(sched.pdl.get_field_idx<Tscal>("psi/ch"));
